...
func (r *mutationResolver) CreateEquipmentProperty(ctx context.Context, input model.NewEquipmentProperty) (*model.EquipmentProperty, error) {
	...
	res, err := db.Exec("INSERT INTO equipment_property (id, description, address, equipment_id) VALUES ($1, $2, $3, $4)", input.ID, input.Description, input.Address, input.Equipment.ID)
	if err != nil {
		return nil, err
	}
	_, err = res.RowsAffected()
	...
	rows, err = db.Query("SELECT id, description, address, equipment_id FROM equipment_property WHERE id = $1", input.ID)
	defer rows.Close()
	...
	for rows.Next() {
		...
		e := &model.Equipment{}
		if err := rows.Scan(&ep.ID, &ep.Description, &ep.Address, &e.ID); err != nil {
...
func (r *queryResolver) EquipmentProperty(ctx context.Context, filter *model.FilterEquipmentProperty) ([]*model.EquipmentProperty, error) {
	...
	if filter.Equipment == nil && filter.ID == nil {
		rows, err = db.Query("SELECT id, description, address, equipment_id FROM equipment_property")
	} else if filter.Equipment == nil && filter.ID != nil {
		rows, err = db.Query("SELECT id, description, address, equipment_id FROM equipment_property WHERE id = $1", *filter.ID)
	} else if filter.Equipment != nil && filter.ID == nil {
		rows, err = db.Query("SELECT id, description, address, equipment_id FROM equipment_property WHERE equipment_id = $1", *&filter.Equipment.ID)
	} else {
		rows, err = db.Query("SELECT id, description, address, equipment_id FROM equipment_property WHERE id = $1 and equipment_id = $2", *filter.ID, filter.Equipment.ID)
	}
